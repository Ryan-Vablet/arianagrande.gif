"""Global hotkey listener for automation binds (works when app does not have focus).

Uses keyboard.hook (low-level) instead of add_hotkey so that bound keys are
detected even when other keys (W, right-click) are held during gameplay.
"""
...
class _ListenerThread(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            logger.warning("keyboard library not installed; global hotkey disabled.")
            return
        ...
        while self._running:
            ...
            try:
                ...
                for bind in binds:
                    ...
                    key_to_bind.setdefault(parsed[1], set()).add(bind)
                ...
                def on_event(event):
                    ...
                    if not key_normalized:
                        return
                    is_mod = is_modifier_token(key_normalized)
                    if event.event_type == keyboard.KEY_DOWN:
                        ...
                        for b in key_to_bind.get(key_normalized, set()):
                            active_triggers.discard(b)

                self._hook = keyboard.hook(on_event)
            except Exception as e:
                logger.debug("keyboard hook failed: %s", e)
...
class CaptureOneKeyThread(QThread):
    """Captures the next keyboard combo and emits it as a bind string."""
